...
class ObjectAnchorVisualization {
    ...
    init(for anchor: ObjectAnchor, withModel model: Entity? = nil, withState: AppModel) {
...
//            model.applyMaterialRecursively(wireframeMaterial)
            // Use the model as occlusion object where passthrough is used.
            model.applyMaterialRecursively(OcclusionMaterial())
                            
//            for cake: Entity in withState.cakeLoader.cakes {
//                entity.addChild(cake)
//            }
...
//        boundingBoxOutline.entity.isEnabled = true
    ...
    func loadCakes() async -> Entity?  {
        var model: ModelEntity? = nil
        do {
            model = try await ModelEntity(named: "schuim_cake") // no .usdz extension needed
        } catch {
